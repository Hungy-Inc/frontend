...
export default function EditScheduledEmail() {
  ...
  const [formData, setFormData] = useState<ScheduledEmail | null>(null);
  ...
  const fetchScheduledEmail = async () => {
    try {
      setFetching(true);
      const data = await api.getScheduledEmail(id);
      const email = data.scheduledEmail;
      setFormData(email);
      // Determine if using template or custom content
      setUseTemplate(!!email.templateId && !email.htmlContent);
    } catch (error) {
      console.error('Error fetching scheduled email:', error);
      toast.error('Error fetching scheduled email');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.updateScheduledEmail(id, payload);
      toast.success('Scheduled email updated successfully');
      router.push('/email-management/scheduled');
    } catch (error: any) {
      console.error('Error updating scheduled email:', error);
      toast.error(error.message || 'Error updating scheduled email');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900">Edit Scheduled Email</h1>
        <p className="text-gray-600 mt-2">Update scheduled email configuration</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border p-6 space-y-6">
              ...
              <div className="flex items-center space-x-4 mb-4">
                <label className="flex items-center">
                  <input
                    ...
                    name="useTemplate"
                    checked={useTemplate}
                    onChange={(e) => setUseTemplate(true)}
                ...
                <label className="flex items-center">
                  <input
                    ...
                    name="useTemplate"
                    checked={!useTemplate}
                    onChange={(e) => setUseTemplate(false)}
                    className="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300"
                  />
        ...
        <div className="flex items-center justify-end space-x-4 pt-4 border-t">
          ...
          >
            ...
            {loading ? 'Updating...' : 'Update Scheduled Email'}
          </button>
        </div>
      </form>
    </div>

      ...
      <div className={styles.tableWrapper}>
        <div className={styles.tableContainer} style={{ overflowX: 'auto' }}>
          {loading ? (
            ...
            <table className={`${styles.table} ${styles.colScroll}`} style={{ width: donationLocations.length + 2 <= 6 ? '100%' : `${(donationLocations.length + 2) * 16.6667}%` }}>
              <thead>
                <tr>
                ...
                {categories.map((cat: string) => (
                  <tr key={cat}>
                    ...
                    {donationLocations.map((donationLocation: string) => (
                      <td key={donationLocation}>{formatWeight(tableData[donationLocation][cat]).toFixed(2)}</td>
                    ))}
